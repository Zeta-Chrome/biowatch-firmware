...
void barometer_task(void *user_data)
{
    ...
    while (1)
    {
        ...
        BW_LOG("Pressure: %d.%d, temperature: %d.%d\n", press / 10, press % 10, temp / 10, temp % 10);
        rtos_task_delay(5000);
    }
}
...
int main()
{
    ...
    rtos_conf_t conf = {.pool_confs = {{.sz = MEM_BLOCK_SZ_2048, .count = 2},
    ...
    rtos_task_create(barometer_task, "Button Task", 7, 128, NULL, &handle0);
